...
public class DeleteGpuController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// dao
		gpuDao = new GpuDao();
		int row = gpuDao.deleteGpu(gpuNo);

		if(row == 1) { // 삭제 성공 시, List로 돌아가기
			System.out.println("[DeleteGpuController.doGet] : Gpu 삭제 성공");
			response.sendRedirect(request.getContextPath() + "/AdminGpuListController");
			return;
		} else { // 삭제 실패해도 List로 돌아가기
			System.out.println("[DeleteGpuController.doGet] : Gpu 삭제 실패");
			response.sendRedirect(request.getContextPath() + "/AdminGpuListController");
			return;
		}
	}
